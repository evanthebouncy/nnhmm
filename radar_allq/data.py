...
def gen_data(n_batch = N_BATCH):
  ...
  # coordinatrs
  new_obs = [[] for _ in range(L*L)]
  new_ob_tfs = [[] for _ in range(L*L)]
  ...
  for bb in range(n_batch):
    ...
    for iii in range(L):
      for jjj in range(L):
        qryy = mk_query(x_coord)
        _new_ob_coord = (iii,jjj)
        _new_ob_lab = qryy(_new_ob_coord)
        _new_ob_x, _new_ob_y = vectorize(_new_ob_coord)
        _new_ob = np.concatenate([_new_ob_x, _new_ob_y])
        new_obs[iii*L+jjj].append(_new_ob)
        new_ob_tfs[iii*L+jjj].append(_new_ob_lab)
  ...
  return  np.array(x_x, np.float32),\
          ...
          np.array(new_obs, np.float32),\
          np.array(new_ob_tfs, np.float32)
