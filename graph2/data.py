...
def get_instance(test=False):
  ...
  shortest_paths = get_shortest_paths(ge_fail)
  ...
  for i in range(L):
    for j in range(L):
      ...
      if ge_fail[i][j] == 1 and G_E[i][j] == 1:
        fake_img[i][j] = -1
  return fake_img, shortest_paths, ge_fail 
  # img, _x = gen_crack()
...
def rand_data(epi):
  ...
  for bb in range(N_BATCH):
    ...
    qry = mk_query(_x)
    partial_obs = np.zeros([L,L,N])
    full_obs = np.zeros([L,L,N])
    for i in range(L):
      for j in range(L):
        full_obs[i][j] = qry((i,j))
        if np.random.random() < epi:
          partial_obs[i][j] = qry((i,j))
          partial_obs[j][i] = qry((i,j))
