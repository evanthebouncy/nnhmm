...
def entropy(vecc):
  vecc = np.array(vecc)
  return -np.sum(np.log(vecc) * vecc)
...
class Implynet:
  ...
  def __init__(self, name):
    with tf.variable_scope('imply') as scope:
      # set up placeholders
      self.partial_obs = tf.placeholder(tf.float32, [N_BATCH, L , L , N], name="partial_obs")
      self.full_obs = tf.placeholder(tf.float32, [N_BATCH, L , L , N], name="full_obs")

      # some constants
      self.n_hidden = 200

      # make hidden represnatation
      W1 = weight_variable([L * L * N, self.n_hidden])
      b1 = bias_variable([self.n_hidden])

      W2 = weight_variable([self.n_hidden, self.n_hidden])
      b2 = bias_variable([self.n_hidden])

      partial_flat = tf.reshape(self.partial_obs, [N_BATCH, L * L * N])
      hidden = tf.nn.relu(tf.matmul(partial_flat, W1) + b1)
      hidden = tf.nn.relu(tf.matmul(hidden, W2) + b2)

      W_preds = [weight_variable([self.n_hidden, N]) for _ in range(L*L)]
      b_preds = [bias_variable([N]) for _ in range(L*L)]
      e2 = tf.constant(1e-10, shape=[N_BATCH, N])
      ...
      assert 0
      full_obs_split = tf.reshape(full_obs_trans, [L*L, N_BATCH, N])
...
#     print qry_costs[0]
  ...
  def get_all_preds(self, sess, obs):
    ...
    predzz0 = np.reshape(predzz0, [L,L,N])
    return predzz0
  ...
  def get_most_confuse(self, sess, obs):
    ...
    most_confs = [(entropy(x[1]), x[0]) for x in all_pred_at_key1]
    most_conf = max(most_confs)
    return most_conf[1]
