...
class Implynet:
  ...
  def __init__(self, name):
    with tf.variable_scope('imply') as scope:
      # set up placeholders
      self.ph_obs_x = [tf.placeholder(tf.float32, [N_BATCH, L * L], 
      ...
      self.ph_new_ob_x = tf.placeholder(tf.float32, [N_BATCH, L * L], name="ph_new_ob_x")
      self.ph_new_ob_tf = tf.placeholder(tf.float32, [N_BATCH,2], name="ph_new_ob_tf")


      # some constants
      self.n_hidden = 1000
      self.n_pred_hidden = 500
      ...
      # initialize some weights
      # stacked lstm
      cell_n = self.n_hidden / 4
      lstm = tf.nn.rnn_cell.MultiRNNCell([tf.nn.rnn_cell.LSTMCell(cell_n), tf.nn.rnn_cell.LSTMCell(cell_n)])
    ...
    with tf.variable_scope('imply') as scope:
      ...
      hidden_cat_query = [tf.nn.relu(\
        tf.matmul(tf.concat(1, [self.ph_new_ob_x, hidden]),W_query1) + b_query1)\
  ...
  def train(self, sess, data_batch):
    _, obs_x, obs_tfs, new_ob_x, new_ob_tf, _ = data_batch
    feed_dic = self.gen_feed_dict(obs_x, obs_tfs, new_ob_x, new_ob_tf)
  ...
  def get_preds_batch(self, sess, obs, batch_querys):
    ...
    for q in batch_querys:
      new_ob_x.append(vectorize_flat(q))
